...
SELECT COUNT(*) count, category FROM analytics GROUP BY category;
...
SELECT app_name, MAX(reviews) AS reviews FROM analytics WHERE rating >= 4.8 GROUP BY app_name;
...
SELECT app_name, reviews, price FROM analytics WHERE price BETWEEN .10 and 1.00 AND rating IS NOT NULL ORDER BY reviews desc LIMIT 10;
